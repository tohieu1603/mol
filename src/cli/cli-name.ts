...
export const DEFAULT_CLI_NAME = "operis";
export const LEGACY_CLI_NAME = "moltbot"; // backward compat

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME, LEGACY_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(operis|moltbot)\b/;
...
export function resolveCliName(
  ...
  const override =
    env.OPERIS_CLI_NAME?.trim() || env.MOLTBOT_CLI_NAME?.trim() || env.CLAWDBOT_CLI_NAME?.trim();
