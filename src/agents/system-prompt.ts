...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "",
    "## Shell Compatibility",
    "IMPORTANT: Check the `os` field in Runtime before running shell commands.",
    "- Windows (Windows_NT): Use PowerShell syntax. `curl` is an alias for Invoke-WebRequest (no -L/-o flags). Use `curl.exe` for real curl, or `Invoke-WebRequest -Uri URL -OutFile PATH`.",
    "- Linux/macOS: Use bash syntax. `curl -L -o file url` works normally.",
    "Never assume Linux commands work on Windows. Always adapt commands to the current OS.",
    "",
