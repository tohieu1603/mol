...
function linkGlobalBinary(repoRoot) {
  // Skip if installed as a dependency (not cloned repo)
  const isClonedRepo = fs.existsSync(path.join(repoRoot, ".git"));
  if (!isClonedRepo) return;
  ...
  try {
    if (pm === "pnpm") {
      // Check if pnpm global bin dir exists
      const checkResult = spawnSync("pnpm", ["root", "-g"], {
        cwd: repoRoot,
        stdio: "pipe",
        encoding: "utf-8"
      });
      ...
      if (checkResult.status !== 0) {
        ...
        console.log("[postinstall] Setting up pnpm global bin...");
        spawnSync("pnpm", ["setup"], { cwd: repoRoot, stdio: "inherit" });
      }

      // Link globally
      console.log("[postinstall] Linking operis globally...");
      const linkResult = spawnSync("pnpm", ["link", "--global"], {
        cwd: repoRoot,
        stdio: "inherit"
      });

      if (linkResult.status === 0) {
        console.log("[postinstall] ✓ operis linked globally. Run: operis --help");
      }
    } else if (pm === "npm" || pm === "bun") {
      console.log("[postinstall] Linking operis globally...");
      spawnSync("npm", ["link"], { cwd: repoRoot, stdio: "inherit" });
      console.log("[postinstall] ✓ operis linked globally. Run: operis --help");
    }
  } catch (err) {
    console.warn(`[postinstall] Global link failed (non-fatal): ${err.message}`);
  }
}
